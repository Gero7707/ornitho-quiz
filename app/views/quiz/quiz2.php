...
<main class="img-fond">
    ...
    <section class="display-cards display-quiz">
        <h1 class="titre-quiz">Vrai ou Faux</h1>
        
        <div class="quiz-container" x-data="audioManager()">
            <?php if(!isset($resultat)): ?>
                <h2 class="sous-titre-quiz">Ce son est-il un chant ou un cri ?</h2><br>
                <form action="/quiz/validerJeu2" method="POST">
                    <input type="hidden" name="bonne_reponse" value="<?= htmlspecialchars($chantOuCri['type_son']) ?>">
                    <input type="hidden" name="oiseau_id" value="<?= htmlspecialchars($oiseau['id']) ?>">
                    <input type="hidden" name="son_id" value="<?= htmlspecialchars($chantOuCri['id']) ?>">
                    
                    <div class="quiz-item" x-data="{ idx: 0 }"> 
                        <div class="player" :class="{ playing: currentIdx === idx }">
                            <audio x-ref="audio_0"
                                src="<?= htmlspecialchars(getAudioUrl($chantOuCri['chemin_fichier'])) ?>"
                                preload="metadata"
                            ...
                            <div class="player-progress-wrap"
                                @click="seek($event, $refs['audio_0'], idx)">
                    ...
                    <div class="validation">
                        <button type="submit" name="reponse" value="Chant" class="valider">Chant</button>
                        <button type="submit" name="reponse" value="Cri" class="valider">Cri</button>
                    </div>
                </form>
            ...
            <?php else: ?>
                <div class="result-navigation">
                    ...
                    <?php if($resultat === 'correct'): ?>
                        <p class="bonne-reponse">Bonne réponse !</p><br>
                        <p class="bonne-reponse-texte">Bravo, c'était un <?= htmlspecialchars($son['type_son']) ?> !</p>
                    <?php else: ?>
                        <p class="mauvaise-reponse">Mauvaise réponse...</p>
                        <p class="mauvaise-reponse-texte">Dommage ! C'était en fait un <?= htmlspecialchars($son['type_son']) ?>.</p>
                    <?php endif; ?>
