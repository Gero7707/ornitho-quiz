"""
Compare les espèces d'oiseaux disponibles sur Xeno-canto (Guyane française)
avec les espèces déjà en BDD OrnithoQuizz.

Résultat : liste des espèces présentes sur XC mais absentes de la BDD.

Usage : python scripts/comparer_especes_guyane.py
"""
...
# ─── ÉTAPE 1 : Récupérer toutes les espèces XC pour la Guyane ────────────────

print("=== Récupération des espèces Xeno-canto (Guyane française) ===\n")
...
while True:
    params = {
        "query": f"box:{BOX_GUYANE} grp:birds",
        "page":  page,
        "key":   XENO_API_KEY,
    }
...
print(f"\n\n✅ Total espèces XC Guyane : {len(especes_xc)}\n")
...
print("=== Espèces présentes sur XC Guyane mais absentes de la BDD ===\n")
...
with open("especes_manquantes_guyane.csv", "w", encoding="utf-8") as f:
    f.write("nom_latin,nom_anglais\n")
    for nom_latin, en in sorted(manquantes.items(), key=lambda x: x[1]):
        f.write(f'"{nom_latin}","{en}"\n')

print(f"\n💾 Résultats exportés dans especes_manquantes_guyane.csv")
