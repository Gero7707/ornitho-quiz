"""
Import des espèces de Guyane française
=======================================
Pour chaque espèce dans especes_a_importer_guyane.csv :
  1. Crée la fiche en BDD (table oiseaux) avec region='guyane' si elle n'existe pas
  2. Interroge l'API Xeno-canto (box Guyane française)
  3. Télécharge les MP3
  4. Les uploade sur Cloudflare R2
  5. Insère les métadonnées en BDD (table sons)

Usage : python scripts/import_especes_guyane.py
"""
...
# Lire le CSV des espèces à importer
especes = []
with open("especes_a_importer_guyane.csv", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    for row in reader:
        especes.append({
            "nom_latin":   row["nom_latin"].strip(),
            "nom_anglais": row["nom_anglais"].strip(),
        })

# ── TEST : décommenter pour tester sur 3 espèces seulement ──
# especes = especes[:3]

print(f"=== Import espèces Guyane française ===")
...
for espece in especes:
    ...
    if not oiseau:
        cursor.execute("""
            ...
            VALUES (%s, %s, 'guyane')
        """, (nom_anglais, nom_latin))
        conn.commit()
        oiseau_id  = cursor.lastrowid
        nom_commun = nom_anglais
        total_especes_creees += 1
        print(f"   ➕ Fiche créée (nom commun provisoire : {nom_anglais})")
    else:
        oiseau_id  = oiseau["id"]
        nom_commun = oiseau["nom_commun"]
        # Si l'espèce existait déjà (métropole par ex.), on met à jour la region
        # seulement si elle était 'metropole' — à adapter selon ton besoin
        print(f"   ✅ Fiche existante : {nom_commun} (region inchangée)")
    ...
    params = {
        "query": f"gen:{genre} sp:{sp} box:{BOX_GUYANE}",
        "page":  1,
        "key":   XENO_API_KEY,
    }
    ...
    if not enregistrements:
        print(f"   ⚠️  Aucun enregistrement trouvé en Guyane")
        total_ignores += 1
        time.sleep(PAUSE)
        continue
